---
name: oanastack
description: oanastack 协作规则与常见工作流（调研、开 feature、修 bug、验证、review、盯 PR/CI、清理 worktree、按门槛复盘）；以及用 oana grokc 驱动本地 Codex（launch/reply/watch/approve、审批转人工、并发 session）。不要用本 skill 替代 Cloud Agent remote、没人要求就 npm publish、homemade / 第二套 worktree、独立 grokc CLI 仓库、或自动 approve（除非用户明确要求）。
---
...
- 长 coding session / 本地 Codex（`oana grokc`）：`references/grokc.md`
- 功能开发参考：`references/feature.md`
- UI 状态（数据源 / 建模 / Electron）：`references/ui-state.md`
- UI oxlint 循环（模板）：`references/ui-state-oxlint.md`（`references/templates/oxlintrc.react.json`）
- UI React Doctor（Agent 可脚本循环）：`references/ui-state-react-doctor.md`
- UI react-scan（运行时高亮）：`references/ui-state-react-scan.md`
...
多仓 feature 推荐先用 `oana` CLI 创建/清理 worktree，再把 session cwd 指向对应 worktree。长 coding session 在该 worktree 内用 `oana grokc`（细则：`references/grokc.md`）；不要另起一套 worktree 平面。grokc 的产品入口只在本仓（`packages/grokbot-coding-agent/` + `oana grokc`），不要指向独立 grokc CLI 仓库。
