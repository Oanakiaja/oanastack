...
多仓 feature 推荐先用 `oana` CLI 创建/清理 worktree，再把 session cwd 指向对应 worktree。长 coding session 在该 worktree 内用 `oana grokc`（skill：`../grokc-local-coding-agents/SKILL.md`）；不要另起一套 worktree 平面。
