...
薄 playbook 层：路由 / 调研先于动手 / CI 先分类再重试 / review 与 babysit 分离 / worktree 清理 / 按门槛复盘。
...
共用纪律：禁止直接改生产环境配置；默认 `gh`，不要 Graphite；worktree 只用 `oana` / `wt`。长 coding session 用 `oana grokc`，仍挂在同一 worktree 平面（见 `../../grokc-local-coding-agents/SKILL.md`）。
