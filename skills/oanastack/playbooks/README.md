...
薄 playbook 层：路由 / 调研先于动手 / UI 先定风格 token 与基础组件 / CI 先分类再重试 / review 与 babysit 分离 / worktree 清理 / 按门槛复盘。
...
| 定风格 / 落 token / 基础组件 / 新/改表面 / 「看起来不对」 | `uidesign.md`（foundation `../references/ui-foundation.md`） |
| 复现后的最小修复 | `bugfix.md` |
| 计划并跑目标验证 | `test.md` |
...
- `uidesign.md` — 先定风格 / token / 基础组件，再画表面；实现交 `feat.md`。
...
配套参考卡：`../references/ci-classify.md`、`../references/feature-session.md`、`../references/bug-fix.md`、`../references/reflect.md`、`../references/ui-foundation.md`、`../references/ui-state.md`。
