...
**你只负责把 merge frontier 推到 merge-ready。** 先声明模式，一次清一个 PR，停在人决定 merge 的那条线之前。

匹配：「盯这个 PR」「变绿」「merge-ready」「看 CI」「处理 review / bugbot 评论」「check on PR X」。开 PR 本身不自动进入本卡。只看 diff、写评论走 `review.md`。纯 CI 分诊走 `cicd.md`。

merge / land / ship **不要**在这里做：本卡到 merge-ready 为止，合入由 owner 做。没有 shipping 卡。
...
5. **CI 先分类再重试。** 见 `cicd.md` 与 `../references/ci-classify.md`。禁止盲 `re-run`。
6. **停在 merge-ready。** 不 `gh pr merge`，不 arm merge-when-ready，不代人点 merge。Owner 审批是等待，不是你去绕过的 blocker。
...
- 不 merge、不 arm merge-when-ready。merge / land / ship 是 owner-only。
