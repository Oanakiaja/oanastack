...
1. **开 feature session + worktree。** `oana feature start <slug> [repo-alias...]`；用 `oana feature hint <slug>` 取线程名与 cwd。session cwd 必须指向 worktree，禁止在主 checkout 上开发。长 session 在该 cwd 内用 `oana grokc launch|reply|…`（见 `../references/grokc.md`），不要另起 homemade worktree。
...
- 长 session 编码：`oana grokc`（在 feature worktree cwd 内；细则 `../references/grokc.md`）。`oana grokc worktree` 仍走 `wt`，不是第二套隔离平面。不要把独立 grokc CLI 仓库当产品入口。
